...
public class UserServiceImpl implements UserService {
     ...
     */
    public UserExecution login(String userAcount, String userPwd) {
        try {
        TbUserEntity tbUserEntity = userMapper.selectUser(userAcount,userPwd);
            if(tbUserEntity == null ){
                return  new UserExecution(userAcount,userPwd, UserStateEnum.NO_USER);
            }else{
                return new UserExecution(userAcount,userPwd,UserStateEnum.SUCCESS_LOGIN);
            }
        } catch (Exception e) {
            logger.error(e.getMessage(), e);
            // 所有编译期异常转换为运行期异常
            return new UserExecution(userAcount,userPwd, UserStateEnum.INNER_ERROR);
        }
